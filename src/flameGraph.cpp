...
void FlameGraph::dump(dumpstream& out, bool tree) {
...
void FlameGraph::printFrame(dumpstream& out, u32 key, const Trie& f, int level, u64 x) {
...
void FlameGraph::printTreeFrame(dumpstream& out, const Trie& f, int level, const char** names) {
    std::vector<Node> children;
    children.reserve(f._children.size());
    for (std::map<u32, Trie>::const_iterator it = f._children.begin(); it != f._children.end(); ++it) {
...
void FlameGraph::printCpool(dumpstream& out) {
    out << "'all'";
...
const char* FlameGraph::printTill(dumpstream& out, const char* data, const char* till) {
